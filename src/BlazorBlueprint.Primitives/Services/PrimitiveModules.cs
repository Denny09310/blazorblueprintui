...
public static class PrimitiveModules
{
    ...
    /// </para>
    /// </remarks>
    // Asset revision 4 updates the entry URL for the new swipe-area module, including local
    // builds that retain the same informational assembly version.
    public static string ModuleUrl { get; } = JsModules.Versioned($"{ModulePath}?assets=4", typeof(PrimitiveModules).Assembly);
