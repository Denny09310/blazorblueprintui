...
- **Consumer `group-*` and `peer-*` variants reach `BbSidebar`.** Tailwind resolves those variants against a literal marker class, and a consumer's own build emits `:where(.group)` — never `:where(.bb\:group)`, because it never sees the `bb` prefix. The sidebar carried only the prefixed pair, so the documented `group-data-[collapsible=icon]:hidden` silently matched nothing and a collapsed sidebar kept the labels and chevrons the consumer had asked it to hide. The bare `group` and `peer` markers now sit alongside the prefixed ones.
